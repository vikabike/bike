...
$meta_boxes = array(
    'Product' => array(
        'name' => 'image1',
        'title' => __('First Image URL (Optional)', 'salejunction'),
        'description' => __('Enter first image URL.', 'salejunction'),
    ),
    'Paypal' => array(
        'name' => 'image2',
        'title' => __('Second Image URL(Optional)', 'salejunction'),
        'description' => __('Enter second image URL.', 'salejunction')
    ),
    'ThankURL' => array(
        'name' => 'image3',
        'title' => __('Third Image URL (Optional)', 'salejunction'),
        'description' => __('Enter third image URL.', 'salejunction')
    ),
    'CancelURL' => array(
        'name' => 'see_demo',
        'title' => __('See Demo URL (Optional)', 'salejunction'),
        'description' => __('Enter your demo URL.', 'salejunction')
    )
);
...
function create_meta_box() {
    ...
    if (function_exists('add_meta_box'))
        add_meta_box('new-meta-boxes', __('Single Page Slider Images and Demo URL Meta', 'salejunction'), 'display_meta_box', 'download', 'normal', 'high');
}
                    ...
                    switch ($meta_box['type']) :
                        case "datetime" :
                            if ($post->post_status <> 'publish') :
                                echo '<p>' . __('Post is not yet published', 'salejunction') . '</p>';
                            else :
                                $date = $data;
                                if (!$data) {
                        ...
                        default :
                            ?>
                            <input type="text" style="width:320px; margin-right: 10px; float:left" name="<?php echo $meta_box['name']; ?>" value="<?php echo htmlspecialchars($data); ?>" /><?php
                            if ($meta_box['description'])
                                echo wpautop(wptexturize($meta_box['description']));
                            ?>
                            <?php
