		...
		<?php while ( $modality_slider_query->have_posts() ): $modality_slider_query->the_post(); ?>
			<?php if ($slider_cat !='') { ?>
					<?php if ( has_post_thumbnail() ) { ?>
						...
						<img src="<?php echo esc_url($image[0]); ?>" title="<?php the_title_attribute(); ?>" alt="<?php esc_html(the_excerpt()); ?>" link="<?php esc_url(the_permalink()); ?>" />
					<?php } else { ?>
						<img class="attachment-full wp-post-image rs-slide-image" alt="slide" src="<?php echo esc_url(get_template_directory_uri().'/images/assets/slide.jpg');?>">
					<?php } ?>
			<?php } else { ?>
				<?php if ( has_post_thumbnail() ) { ?>
				...
				<?php } else { ?>
					<?php if ($slider_cat !='') { ?>
						<img class="attachment-full wp-post-image rs-slide-image" alt="slide" src="<?php echo esc_url(get_template_directory_uri().'/images/assets/slide.jpg');?>">
					<?php } else { ?>
						<img class="attachment-full wp-post-image rs-slide-image" alt="slide" src="<?php echo esc_url(get_template_directory_uri().'/images/assets/slide1.jpg');?>">
					<?php } ?>
				<?php } ?>
			<?php } ?>
		...
		<?php while ( $modality_slider_query->have_posts() ): $modality_slider_query->the_post(); ?>
			...
			<?php if ($modality_theme_options['captions_on'] == '1') { ?>	
				...
				<?php if ($modality_theme_options['captions_button'] == '1') { ?>
					<a href="<?php esc_url(the_permalink()); ?>" class="btn"><?php echo $button_text ?></a>
				<?php }; ?>
			<?php }; ?>			
			</li>
		<?php endwhile; wp_reset_postdata(); ?>
...
function modality_localize_scripts_ideal(){
	...
	if ($modality_theme_options['captions_on'] == '1') {
		if (is_home() && ! is_paged()) {
			wp_enqueue_script( 'modality-slides-captions', get_template_directory_uri() .'/js/slides-captions.js' , array( 'jquery' ), '', true );
			wp_localize_script( 'modality-slides-captions', 'php_vars', $datatoBePassed );
		}
	}else{
		wp_enqueue_script( 'modality-ideal-slides', get_template_directory_uri() .'/js/ideal-slides.js' , array( 'jquery' ), '', true );
		wp_localize_script( 'modality-ideal-slides', 'php_vars', $datatoBePassed );
	}
...
function modality_localize_scripts_unslider(){
	wp_enqueue_script( 'modality-slides', get_template_directory_uri() .'/js/slides.js' , array( 'jquery' ), '', true );
	$modality_theme_options = modality_get_options( 'modality_theme_options' );
	$animation_speed = $modality_theme_options['animation_speed'];
	$slideshow_speed = $modality_theme_options['slideshow_speed'];
		$datatoBePassed = array(
	...
	wp_localize_script( 'modality-slides', 'php_vars', $datatoBePassed );
}
