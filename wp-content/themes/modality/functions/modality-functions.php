<?php
/**
 * Modality functions and defmodalityns
 *
 * @package Modality
 *
 *
...
function modality_add_script_function() {
	...
	*/
	...
	if ($modality_theme_options['responsive_design'] == '1'):
		wp_enqueue_style ('modality-responsive', get_template_directory_uri() . '/css/responsive.css');
	endif;
	wp_enqueue_style ('font-awesome', get_template_directory_uri() . '/css/font-awesome.css');
	if( $modality_theme_options['google_font_body'] !=""):
		wp_enqueue_style ('modality-body-font', '//fonts.googleapis.com/css?family='. urlencode($modality_theme_options['google_font_body']) .':400,400italic,700,700italic&subset=latin,greek-ext,cyrillic,latin-ext,greek,cyrillic-ext,vietnamese');
	endif;
	if( $modality_theme_options['google_font_menu'] != ""):
		wp_enqueue_style ('modality-menu-font', '//fonts.googleapis.com/css?family='. urlencode($modality_theme_options['google_font_menu']) .':400,400italic,700,700italic&subset=latin,greek-ext,cyrillic,latin-ext,greek,cyrillic-ext,vietnamese');
	endif;
	if( $modality_theme_options['google_font_logo'] != ""):
		wp_enqueue_style ('modality-logo-font', '//fonts.googleapis.com/css?family='. urlencode($modality_theme_options['google_font_logo']) .':400,400italic,700,700italic&subset=latin,greek-ext,cyrillic,latin-ext,greek,cyrillic-ext,vietnamese');
	endif;
	...
	wp_enqueue_script('modality-custom', get_template_directory_uri() . '/js/custom.js', array( 'jquery' ),'', true);	
	...
	if ( $modality_theme_options['menu_sticky'] == 1) {
		...
		wp_enqueue_script('modality-sticky', get_template_directory_uri() . '/js/sticky.js', array( 'jquery' ),'', false);
	}
	if ( $modality_theme_options['scrollup'] == 1) { 
		wp_enqueue_script('modality-scrollup', get_template_directory_uri() . '/js/scrollup.js', array( 'jquery' ),'', true); 
	}
	...
	wp_enqueue_script( 'modality-html5', get_template_directory_uri() . '/js/html5.js', array(), '' );
	wp_script_add_data( 'modality-html5', 'conditional', 'lt IE 9' );
}
